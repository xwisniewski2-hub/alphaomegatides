...
function ChatBot({onClose}){
  ...
  const suggestions=["What is BPC-157?","How do I reconstitute GLP-3R?","What's the difference between CJC-1295 and Ipamorelin?","Can I stack TB-500 with BPC-157?"];

  async function send(text){
    const t=(text||input).trim(); if(!t||loading)return;
    sm(p=>[...p,{from:"user",text:t}]); si(""); setSS(false); setLoading(true);
    const history=[...msgs,{from:"user",text:t}].filter((_,i)=>i>0).map(m=>({role:m.from==="user"?"user":"assistant",content:m.text}));
    const apiMsgs=[];let lastRole=null;
    for(const m of history){if(m.role!==lastRole){apiMsgs.push(m);lastRole=m.role;}else{apiMsgs[apiMsgs.length-1].content+="\n"+m.content;}}
    ...
    try{
      ...
      sm(p=>[...p,{from:"bot",text:data.content?.[0]?.text||"Sorry, couldn't process that. Try again."}]);
    }catch(e){sm(p=>[...p,{from:"bot",text:"Connection issue — please try again."}]);}
    setLoading(false);
  }
      ...
      <div style={{background:"#1a1a1a",padding:"13px 16px",display:"flex",alignItems:"center",justifyContent:"space-between",flexShrink:0,borderBottom:"1px solid rgba(255,255,255,0.07)"}}>
        <div style={{display:"flex",alignItems:"center",gap:10}}>
          ...
          <div>
            ...
            <div style={{fontSize:"0.63rem",color:"#3be8b0",display:"flex",alignItems:"center",gap:4}}><span style={{width:5,height:5,borderRadius:"50%",background:"#3be8b0",display:"inline-block"}}/>Powered by Claude AI</div>
          </div>
        </div>
        <button onClick={onClose} style={{background:"transparent",border:"none",color:"rgba(255,255,255,0.5)",fontSize:"1.1rem",cursor:"pointer",padding:4}}>✕</button>
      </div>
      <div style={{flex:1,overflowY:"auto",padding:"14px 14px 8px",display:"flex",flexDirection:"column",gap:10}}>
        {msgs.map((m,i)=>(
          <div key={i} style={{display:"flex",justifyContent:m.from==="user"?"flex-end":"flex-start"}}>
            <div style={{maxWidth:"85%",background:m.from==="user"?"#4f8ef7":"rgba(255,255,255,0.06)",color:"#fff",borderRadius:m.from==="user"?"16px 16px 4px 16px":"16px 16px 16px 4px",padding:"10px 14px",fontSize:"0.83rem",lineHeight:1.55,whiteSpace:"pre-wrap"}}>{m.text}</div>
          </div>
        ))}
        {loading&&<div style={{display:"flex",justifyContent:"flex-start"}}><div style={{background:"rgba(255,255,255,0.06)",borderRadius:"16px 16px 16px 4px",padding:"10px 14px",fontSize:"0.83rem",color:"rgba(255,255,255,0.5)"}}>Thinking...</div></div>}
        {showSuggestions&&<div style={{display:"flex",flexWrap:"wrap",gap:6,marginTop:4}}>{suggestions.map((s,i)=><button key={i} onClick={()=>send(s)} style={{background:"rgba(59,232,176,0.08)",border:"1px solid rgba(59,232,176,0.2)",borderRadius:20,padding:"6px 12px",color:"#3be8b0",fontSize:"0.72rem",cursor:"pointer",fontFamily:"inherit"}}>{s}</button>)}</div>}
        <div ref={bottomRef}/>
      </div>
      <div style={{padding:"10px 12px",borderTop:"1px solid rgba(255,255,255,0.07)",display:"flex",gap:8,flexShrink:0}}>
        <input value={input} onChange={e=>si(e.target.value)} onKeyDown={e=>e.key==="Enter"&&send("")} placeholder="Ask about any peptide..." style={{flex:1,background:"rgba(255,255,255,0.06)",border:"1px solid rgba(255,255,255,0.1)",borderRadius:100,padding:"10px 16px",color:"#fff",fontSize:"0.83rem",outline:"none",fontFamily:"inherit"}}/>
        <button onClick={()=>send("")} disabled={loading} style={{background:"#3be8b0",border:"none",borderRadius:100,padding:"10px 18px",color:"#0e0e0e",fontWeight:700,fontSize:"0.83rem",cursor:"pointer",fontFamily:"inherit"}}>Send</button>
      </div>
    </div>
  );
}
