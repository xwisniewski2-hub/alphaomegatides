...
const PRODUCTS = [
  ...
  {id:"bpc157",name:"BPC-157",tag:"Healing & Recovery",icon:"🧬",color:"#3be8b0",
   ...
   chips:["Angiogenesis","Tissue Biology Research","Gut Integrity"]},
  ...
  {id:"ghkcu",name:"GHK-CU",tag:"Research Compound",icon:"✨",color:"#ffd166",
   ...
   desc:"Copper-binding tripeptide for matrix-signaling, extracellular matrix remodeling, wound biology research, and tissue-response research. 4,000+ gene activations documented. Blue solution upon reconstitution is normal.",
   coa:{purity:"99%+",labs:["Freedom Diagnostics"],tests:[
  ...
  {id:"glow",name:"Glow Complex",tag:"Skin Research",icon:"🌟",color:"#ec4899",
   ...
   desc:"Skin health research blend: GHK-Cu + BPC-157 + Thymosin Beta-4. Three peptides studied for matrix modulation, tissue biology research, and cellular regeneration.",
   coa:{purity:"99%+",labs:["Freedom Diagnostics"],tests:[
...
const BOT_QA = [
  ...
  {q:["research administration site","where to inject","subcutaneous","subq","abdomen","rotate"],
  ...
  {q:["bpc-157","bpc","tb-500","tb500","body protection"],
   a:"BPC-157 and TB-500 are popular research peptides studied for tissue biology research and healing mechanisms. While we don't currently carry these as standalone products, our GHK-Cu compound has well-documented roles in wound biology research, collagen synthesis, and regenerative pathways. Check back — our compound lineup is expanding."},
  // ── COA / Quality ──
...
// Auto-clean fake orders from existing accounts on load
(()=>{try{const u=JSON.parse(localStorage.getItem("nxg_u")||"{}");let ch=false;Object.keys(u).forEach(e=>{if(u[e].orders?.length){u[e].orders=[];ch=true;}});if(ch)localStorage.setItem("nxg_u",JSON.stringify(u));}catch{}})();
...
function Nav({user,go,onLogout,cartCount}){
  ...
  const CATS=[
    {label:"Body Weight Research",ids:["glp3r","glp2t","glp1"]},
    {label:"Recovery Research",ids:["bpc157","tb500","ghkcu","glow"]},
    {label:"Growth Research",ids:["cjc1295","cjcipa","ipamorlin","tesamorlin","igf1lr3","nad","motsc","ss31","glutathione"]},
    {label:"Neuro Research",ids:["selank","semax","dsip","mt2"]},
  ];
  return <>
    {/* Overlay */}
    ...
    <nav style={{position:"fixed",top:0,left:0,right:0,zIndex:998,display:"flex",alignItems:"center",justifyContent:"space-between",padding:"14px 20px",background:"rgba(10,10,10,0.97)",backdropFilter:"blur(10px)",borderBottom:"1px solid rgba(255,255,255,0.07)"}}>
      ...
      <div onClick={(e)=>{if(e.detail===3){go("admin");}else{go("home");close();}}} style={{cursor:"pointer",position:"absolute",left:"50%",transform:"translateX(-50%)",display:"flex",alignItems:"center",gap:4}}>
      ...
      <div style={{display:"flex",alignItems:"center",gap:10}}>
        {/* Cart */}
        <button onClick={()=>{go("coming-soon");close();}} style={{background:"none",border:"none",cursor:"pointer",padding:"6px 8px",position:"relative",minHeight:"auto",borderRadius:8}}>
        ...
        {user
          ...
          :<button onClick={()=>downloadPlacard(data,key||"")} style={{width:"100%",padding:"12px",background:`linear-gradient(135deg,${data.accent}33,${data.accent}11)`,border:`1.5px solid ${data.accent}77`,borderRadius:10,color:data.accent,fontFamily:"'Syne',sans-serif",fontWeight:800,fontSize:"0.82rem",letterSpacing:"0.08em",cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",gap:8,marginBottom:14,transition:"all 0.15s",boxShadow:`0 4px 20px ${data.accent}22`}}
...
const HOME_CATS = [
  ...
  {id:"glp",       label:"Body Weight Research",      emoji:"🔥", ids:["glp3r","glp2t","glp1"]},
...
// ── RESEARCH LIBRARY ─────────────────────────────
const ARTICLES = [
  {id:"glp-intro",title:"Understanding GLP Receptor Agonists",category:"Body Weight Research",emoji:"🔥",time:"5 min read",summary:"How GLP-1, GLP-2, and GLP-3 receptors work differently and why triple agonism makes Retatrutide the most potent body weight research peptide studied to date.",tags:["GLP-3 R","GLP-2 T","GLP-1"]},
  {id:"bpc-tb500",title:"BPC-157 + TB-500: The Ultimate Recovery Stack",category:"Recovery",emoji:"🛡️",time:"4 min read",summary:"Why these two peptides work synergistically — BPC-157 targets gut and tissue biology research while TB-500 drives systemic actin-driven healing. When and how to use both.",tags:["BPC-157","TB-500","Wolverine"]},
  ...
  {id:"ghkcu-skin",title:"GHK-Cu: The Copper Peptide Activating 4,000+ Genes",category:"Skin & Anti-Age",emoji:"✨",time:"5 min read",summary:"GHK-Cu's mechanism on ECM remodeling, collagen synthesis, and how it regulates gene expression. Why it's considered one of the most studied cellular research compounds.",tags:["GHK-CU","Glow Complex"]},
...
// ── ACCORDION MENU ───────────────────────────────
const NAV_CATS = [
  {label:"Body Weight Research",      emoji:"🔥", ids:["glp3r","glp2t","glp1"],                                          catId:"glp"},
  ...
  return <div style={{paddingTop:70,background:"#0e0e0e",color:"#ffffff"}}>
    <section style={{minHeight:"90vh",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",textAlign:"center",padding:"80px 24px 60px",background:"#0a0a0a",position:"relative",overflow:"hidden"}}>
      ...
      <div style={{display:"inline-flex",alignItems:"center",gap:8,background:"rgba(255,255,255,0.07)",border:"1px solid rgba(255,255,255,0.15)",padding:"5px 16px",borderRadius:100,fontSize:"0.77rem",fontWeight:500,color:"rgba(255,255,255,0.7)",marginBottom:28,position:"relative",zIndex:1}}>
        ...
        ⚗️ Research Use Only — Not for Human Use · Not FDA Approved · For Qualified Researchers Only
      </div>
  ...
  return <div style={{paddingTop:70,background:"#0e0e0e",color:"#ffffff"}}>
    ...
    <div style={{maxWidth:760,margin:"0 auto",padding:"32px 24px 0"}}>
      ...
      <div style={{background:"#1c1c1c",borderRadius:18,padding:"22px 24px",border:"1px solid rgba(255,255,255,0.1)",marginBottom:24,boxShadow:"0 2px 24px rgba(0,0,0,0.4)"}}>
        ...
        <ViewingNow productId={p.id} color={p.color}/>
        <ProductComplianceBox productName={p.name}/>
...
function CheckoutPage({product:p, go, user}){
  ...
  return <div style={{paddingTop:70,background:"#0e0e0e",minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center"}}>
    <div style={{textAlign:"center",padding:"40px 24px",maxWidth:440}}>
      ...
      <button onClick={()=>go("coming-soon")} style={{marginTop:24,background:"transparent",color:"rgba(255,255,255,0.3)",border:"none",cursor:"pointer",fontFamily:"inherit",fontSize:"0.82rem"}}>← Back to cart</button>
    </div>
  </div>;
}
    ...
    <div style={{padding:"44px 36px 20px",background:"#0a0a0a"}}>
      <div className="footer-grid" style={{maxWidth:1100,margin:"0 auto",display:"grid",gridTemplateColumns:"2fr 1fr 1fr 1fr",gap:36,marginBottom:36}}>
        ...
        <div>
          ...
          {[
            {label:"🔥 Body Weight Research",catId:"glp"},
...
function CheckoutLink({cart,total,go}:{cart:{id:string}[];total:number;go:(p:string)=>void}){
  return(
    <div>
      <button onClick={()=>go("coming-soon")} style={{width:"100%",padding:"15px",background:"#3be8b0",color:"#0e0e0e",border:"none",borderRadius:100,fontFamily:"'DM Sans',sans-serif",fontWeight:800,fontSize:"0.95rem",cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",gap:8,boxShadow:"0 0 24px rgba(59,232,176,0.2)",marginBottom:8}}>
        <span>🚀</span> Complete Order · ${total.toFixed(2)}
      </button>
      <p style={{textAlign:"center",color:"rgba(255,255,255,0.25)",fontFamily:"'DM Sans',sans-serif",fontSize:"0.72rem"}}>Secure checkout · US fulfillment only</p>
    </div>
  );
}
    ...
    <div style={{maxWidth:640,margin:"0 auto",padding:"40px 20px 120px"}}>
      ...
      {cart.length===0
        ...
        :<>
          ...
          <CheckoutLink cart={cart} total={total} go={go}/>
...
export default function App(){
  ...
  useEffect(()=>{
    let triggered=false;
    const exitHandler=(e:MouseEvent)=>{if(!triggered&&e.clientY<10&&!localStorage.getItem("nxg_exit_seen")){triggered=true;localStorage.setItem("nxg_exit_seen","1");setShowExit(true);}};
    document.addEventListener("mouseleave",exitHandler);
    try{
      const p=new URLSearchParams(window.location.search);
      if(p.get("admin")==="1") spg("admin");
      const se=p.get("survey"),sp=p.get("product");
      if(se&&sp){const all=getNxgSignups();const ex=all.find(s=>s.email===se);if(ex){const cur=ex.productInterests||[];if(!cur.includes(sp))updateNxgInterests(se,[...cur,sp]);}window.history.replaceState({},"","/");}
      const unsub=p.get("unsubscribe");
      if(unsub){localStorage.setItem("nxg_signups",JSON.stringify(getNxgSignups().filter(s=>s.email!==unsub)));alert("Unsubscribed.");window.history.replaceState({},"","/");}
    }catch{}
    const keyHandler=(e:KeyboardEvent)=>{if(e.target instanceof HTMLInputElement||e.target instanceof HTMLTextAreaElement)return;if((e.key==="k"||e.key==="K")&&!e.metaKey)document.querySelector<HTMLInputElement>(".search-input")?.focus();if((e.key==="c"||e.key==="C")&&!e.metaKey)spg("coming-soon");};
    document.addEventListener("keydown",keyHandler);
    return()=>{document.removeEventListener("mouseleave",exitHandler);document.removeEventListener("keydown",keyHandler);};
  },[]);
  ...
  return <div style={{fontFamily:"'DM Sans',sans-serif",background:"#0e0e0e",minHeight:"100vh",color:"#ffffff"}} onTouchStart={pg!=="cart"?onTouchStart:undefined} onTouchEnd={pg!=="cart"?onTouchEnd:undefined}>
    ...
    {pg==="product"&&prod&&<div key={"product-"+prod.id} className="page-fade"><ProductPage p={prod} go={go} onAddToCart={(sz,sp)=>{addToCart(prod,sz,sp);go("coming-soon");}} wishlist={wishlist} toggleWishlist={toggleWishlist}/></div>}
...
// ─── ALL NEW FEATURES ─────────────────────────────────────────────────────────
// Appended cleanly — no nested template literals

const RESEND_KEY="re_gNzYdNZU_4Yx2Y916iJb6dSiBniGRchZF";
const ADMIN_EMAIL="alphaomegatides@yahoo.com";
const SITE_URL="https://www.alphaomegatides.com";
const ADMIN_PW="AlphaOmega2026!";

interface NxgSignup{id:string;firstName:string;lastName:string;email:string;phone?:string;interest?:string;productInterests?:string[];signedUpAt:string;surveyCompletedAt?:string;source:string;}

function getNxgSignups():NxgSignup[]{try{return JSON.parse(localStorage.getItem("nxg_signups")||"[]");}catch{return[];}}
function saveNxgSignup(s:NxgSignup){try{const all=getNxgSignups().filter(x=>x.email!==s.email);localStorage.setItem("nxg_signups",JSON.stringify([s,...all]));}catch{}}
function updateNxgInterests(email:string,interests:string[]){try{const all=getNxgSignups().map(s=>s.email===email?{...s,productInterests:interests,surveyCompletedAt:new Date().toISOString()}:s);localStorage.setItem("nxg_signups",JSON.stringify(all));}catch{}}

async function notifyAdmin(signup:NxgSignup){
  const html="<div style='font-family:sans-serif;background:#0e0e0e;color:#fff;padding:32px;border-radius:12px;max-width:500px'>"
    +"<h2 style='color:#3be8b0;margin-bottom:4px'>New Potential Customer</h2>"
    +"<p style='color:rgba(255,255,255,0.5);font-size:12px;margin-bottom:20px'>alphaomegatides.com &middot; "+new Date().toLocaleString()+"</p>"
    +"<table style='width:100%;border-collapse:collapse'>"
    +"<tr><td style='padding:7px 0;color:rgba(255,255,255,0.45);font-size:13px;width:100px'>Name</td><td style='padding:7px 0;color:#fff;font-size:13px;font-weight:600'>"+signup.firstName+" "+signup.lastName+"</td></tr>"
    +"<tr><td style='padding:7px 0;color:rgba(255,255,255,0.45);font-size:13px'>Email</td><td style='padding:7px 0;color:#3be8b0;font-size:13px'>"+signup.email+"</td></tr>"
    +"<tr><td style='padding:7px 0;color:rgba(255,255,255,0.45);font-size:13px'>Phone</td><td style='padding:7px 0;color:#fff;font-size:13px'>"+(signup.phone||"—")+"</td></tr>"
    +"<tr><td style='padding:7px 0;color:rgba(255,255,255,0.45);font-size:13px'>Interest</td><td style='padding:7px 0;color:#fff;font-size:13px'>"+(signup.interest||"—")+"</td></tr>"
    +"<tr><td style='padding:7px 0;color:rgba(255,255,255,0.45);font-size:13px'>Source</td><td style='padding:7px 0;color:#fff;font-size:13px'>"+signup.source+"</td></tr>"
    +"</table></div>";
  try{await fetch("https://api.resend.com/emails",{method:"POST",headers:{"Content-Type":"application/json","Authorization":"Bearer "+RESEND_KEY},body:JSON.stringify({from:"onboarding@resend.dev",to:ADMIN_EMAIL,reply_to:signup.email,subject:"🧬 New Potential Customer — "+signup.firstName+" "+signup.lastName,html})});}catch{}
}

async function sendSurveyEmail(signup:NxgSignup){
  const products=[
    {id:"glp3r",name:"GLP-3R (Retatrutide)",cat:"Metabolic Research"},
    {id:"glp2t",name:"GLP-2T (Tirzepatide)",cat:"Metabolic Research"},
    {id:"glp1",name:"GLP-1 (Semaglutide)",cat:"Metabolic Research"},
    {id:"tesamorlin",name:"Tesamorlin",cat:"Metabolic Research"},
    {id:"bpc157",name:"BPC-157",cat:"Recovery Research"},
    {id:"tb500",name:"TB-500",cat:"Recovery Research"},
    {id:"ghkcu",name:"GHK-Cu",cat:"Dermal Research"},
    {id:"glow",name:"Glow Complex",cat:"Dermal Research"},
    {id:"cjcipa",name:"CJC-1295 / Ipamorelin",cat:"GH Research"},
    {id:"ipamorlin",name:"Ipamorelin",cat:"GH Research"},
    {id:"cjc1295",name:"CJC-1295",cat:"GH Research"},
    {id:"igf1lr3",name:"IGF-1 LR3",cat:"GH Research"},
    {id:"nad",name:"NAD+",cat:"Longevity Research"},
    {id:"motsc",name:"MOTS-c",cat:"Longevity Research"},
    {id:"ss31",name:"SS-31",cat:"Longevity Research"},
    {id:"glutathione",name:"Glutathione",cat:"Longevity Research"},
    {id:"semax",name:"Semax",cat:"Cognitive Research"},
    {id:"selank",name:"Selank",cat:"Cognitive Research"},
    {id:"dsip",name:"DSIP",cat:"Cognitive Research"},
    {id:"mt2",name:"MT2 (Melanotan 2)",cat:"Other"},
  ];
  const cats=[...new Set(products.map(p=>p.cat))];
  const rows=cats.map(cat=>{
    const ps=products.filter(p=>p.cat===cat);
    return "<tr><td colspan='2' style='padding:10px 0 4px;color:#3be8b0;font-size:11px;font-weight:700;letter-spacing:0.1em;border-top:1px solid rgba(255,255,255,0.06);text-transform:uppercase'>"+cat+"</td></tr>"
      +ps.map(p=>"<tr><td style='padding:5px 0;color:rgba(255,255,255,0.7);font-size:13px'>"+p.name+"</td>"
        +"<td style='padding:5px 0;text-align:right'><a href='"+SITE_URL+"/?survey="+signup.email+"&product="+p.id+"' style='display:inline-block;background:rgba(59,232,176,0.15);color:#3be8b0;border:1px solid rgba(59,232,176,0.3);border-radius:20px;padding:3px 12px;font-size:11px;font-weight:700;text-decoration:none'>Interested ✓</a></td></tr>").join("");
  }).join("");
  const html="<div style='background:#0e0e0e;padding:40px 32px;max-width:560px;margin:0 auto;border-radius:16px;font-family:sans-serif'>"
    +"<div style='text-align:center;margin-bottom:28px'><div style='font-size:2rem;margin-bottom:8px'>⚗️</div>"
    +"<h1 style='color:#fff;font-size:22px;font-weight:900;margin:0 0 8px'>Hi "+signup.firstName+",</h1>"
    +"<p style='color:rgba(255,255,255,0.5);font-size:14px;line-height:1.6;margin:0'>Thank you for joining Alphaomegatides early access. We launch soon with the most transparent research peptide platform in the US.<br><br><strong style='color:#fff'>Which compounds interest you most?</strong> Click any that apply.</p></div>"
    +"<div style='background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.08);border-radius:14px;padding:20px 24px;margin-bottom:24px'><table style='width:100%;border-collapse:collapse'>"+rows+"</table></div>"
    +"<div style='text-align:center;border-top:1px solid rgba(255,255,255,0.06);padding-top:16px'><p style='color:rgba(255,255,255,0.2);font-size:11px;line-height:1.6;margin:0'>Alphaomegatides &middot; Research Use Only &middot; US Only<br><em>\"Where the tide turns for all.\"</em><br><a href='"+SITE_URL+"/?unsubscribe="+signup.email+"' style='color:rgba(255,255,255,0.15)'>Unsubscribe</a></p></div></div>";
  try{await fetch("https://api.resend.com/emails",{method:"POST",headers:{"Content-Type":"application/json","Authorization":"Bearer "+RESEND_KEY},body:JSON.stringify({from:"onboarding@resend.dev",to:signup.email,reply_to:ADMIN_EMAIL,subject:"🧬 What interests you? — Alphaomegatides Research Survey",html})});}catch{}
}

async function sendLaunchBlast(signups:NxgSignup[]):Promise<{sent:number;failed:number}>{
  let sent=0,failed=0;
  for(const s of signups){
    try{
      const interests=(s.productInterests||[]).slice(0,4).map(id=>"<span style='background:rgba(59,232,176,0.08);border:1px solid rgba(59,232,176,0.2);border-radius:8px;padding:4px 12px;color:#3be8b0;font-size:12px;font-weight:700;display:inline-block;margin:3px'>"+id.toUpperCase()+"</span>").join("");
      const html="<div style='background:#0e0e0e;padding:40px 32px;max-width:560px;margin:0 auto;border-radius:16px;font-family:sans-serif;text-align:center'>"
        +"<div style='font-size:3rem;margin-bottom:12px'>🧬</div><h1 style='color:#3be8b0;font-size:28px;font-weight:900;margin:0 0 6px'>We're Live.</h1>"
        +"<p style='color:rgba(255,255,255,0.3);font-style:italic;margin:0 0 24px;font-size:13px'>\"Where the tide turns for all.\"</p>"
        +"<p style='color:rgba(255,255,255,0.6);font-size:15px;line-height:1.7;margin:0 0 20px'>Hi "+s.firstName+" — you're on our early access list. Alphaomegatides is officially open. 99%+ purity, independent COA, US fulfillment.</p>"
        +(interests?"<div style='margin-bottom:20px'>"+interests+"</div>":"")
        +"<a href='"+SITE_URL+"' style='display:inline-block;background:#3be8b0;color:#0e0e0e;border-radius:100px;padding:16px 40px;font-size:16px;font-weight:900;text-decoration:none;margin-bottom:24px'>Shop Research Peptides →</a>"
        +"<p style='color:rgba(255,255,255,0.2);font-size:10px;line-height:1.6;margin-top:20px'>For research use only &middot; Not for human use &middot; Not FDA approved<br><a href='"+SITE_URL+"/?unsubscribe="+s.email+"' style='color:rgba(255,255,255,0.15)'>Unsubscribe</a></p></div>";
      await fetch("https://api.resend.com/emails",{method:"POST",headers:{"Content-Type":"application/json","Authorization":"Bearer "+RESEND_KEY},body:JSON.stringify({from:"onboarding@resend.dev",to:s.email,reply_to:ADMIN_EMAIL,subject:"🧬 We're Live — Alphaomegatides is Open",html})});
...
function downloadPlacard(data:any,key:string){
  const wHTML=data.weeks.map((w:any)=>
    "<div style='background:rgba(255,255,255,0.03);border:1px solid "+data.accent+"22;border-radius:12px;padding:16px;margin-bottom:10px'>"
    +"<div style='display:flex;align-items:center;gap:8px;margin-bottom:8px'>"
    +"<span style='background:"+data.accent+"22;color:"+data.accent+";border:1px solid "+data.accent+"44;font-size:9px;font-weight:800;padding:3px 8px;border-radius:6px'>WK "+w.num+"</span>"
    +"<span style='font-size:14px'>"+w.icon+"</span>"
    +"<span style='color:"+data.accent+";font-weight:800;font-size:10px;letter-spacing:0.1em'>"+w.label+"</span></div>"
    +"<p style='font-size:11px;color:rgba(255,255,255,0.6);line-height:1.55;margin-bottom:10px'>"+w.desc+"</p>"
    +"<div style='display:grid;grid-template-columns:1fr 1fr;gap:12px'>"
    +"<div><div style='color:"+data.accent+";font-weight:700;font-size:9px;letter-spacing:0.12em;margin-bottom:5px'>OBSERVED IN RESEARCH</div>"
    +w.notice.map((n:string)=>"<div style='font-size:10px;color:rgba(255,255,255,0.55);line-height:1.5;margin-bottom:3px'><span style='color:"+data.accent+"'>▸</span> "+n+"</div>").join("")+"</div>"
    +"<div><div style='color:#fbbf24;font-weight:700;font-size:9px;letter-spacing:0.12em;margin-bottom:5px'>RESEARCH NOTES</div>"
    +w.focus.map((f:string)=>"<div style='font-size:10px;color:rgba(255,255,255,0.55);line-height:1.5;margin-bottom:3px'><span style='color:#fbbf24'>▸</span> "+f+"</div>").join("")+"</div>"
    +"</div></div>").join("");
  const mHTML=data.monitor.map((m:string)=>"<span style='background:rgba(255,255,255,0.05);color:rgba(255,255,255,0.55);font-size:10px;padding:4px 10px;border-radius:20px;border:1px solid rgba(255,255,255,0.08)'>"+m+"</span>").join("");
  const html="<!DOCTYPE html><html><head><meta charset='UTF-8'><title>Alphaomegatides — "+key.toUpperCase()+" Research Guide</title>"
    ...
    +"<style>*{margin:0;padding:0;box-sizing:border-box}body{background:#0a0a0a;color:#fff;font-family:'DM Sans',sans-serif;padding:40px 32px}.page{max-width:720px;margin:0 auto}@media print{body{background:#0a0a0a!important;-webkit-print-color-adjust:exact;print-color-adjust:exact}}</style>"
    +"</head><body><div class='page'>"
    +"<div style='display:flex;justify-content:space-between;align-items:center;margin-bottom:32px;padding-bottom:16px;border-bottom:1px solid rgba(255,255,255,0.07)'>"
    +"<div style='font-family:Syne,sans-serif;font-weight:900;font-size:13px;letter-spacing:0.2em;color:rgba(255,255,255,0.4);text-transform:uppercase'>Alphaomegatides<br><span style='font-size:9px;color:rgba(255,255,255,0.2);font-weight:400'>VERIFIED RESEARCH PEPTIDES</span></div>"
    +"<div style='font-size:24px'>⚗️</div></div>"
    +"<div style='margin-bottom:24px'><div style='font-size:10px;letter-spacing:0.2em;color:rgba(255,255,255,0.3);text-transform:uppercase;margin-bottom:6px'>Research Protocol Guide</div>"
    +"<div style='font-family:Syne,sans-serif;font-weight:900;font-size:28px;color:#fff;margin-bottom:4px'>"+key.toUpperCase()+"</div>"
    +"<div style='font-size:12px;color:rgba(255,255,255,0.35);margin-bottom:10px'>"+data.subtitle+"</div>"
    +"<div style='width:48px;height:3px;border-radius:100px;background:"+data.accent+";margin-bottom:16px'></div></div>"
    +wHTML
    +"<div style='background:rgba(255,255,255,0.03);border-radius:10px;padding:14px 16px;margin-bottom:14px;border:1px solid rgba(255,255,255,0.06)'>"
    +"<div style='color:rgba(255,255,255,0.4);font-weight:700;font-size:9px;letter-spacing:0.14em;margin-bottom:8px'>MONITOR WEEKLY</div>"
    +"<div style='display:flex;flex-wrap:wrap;gap:6px'>"+mHTML+"</div></div>"
    +"<div style='background:"+data.accent+"0a;border-left:3px solid "+data.accent+";border-radius:0 10px 10px 0;padding:12px 16px;margin-bottom:14px'>"
    +"<div style='color:"+data.accent+"bb;font-family:Georgia,serif;font-style:italic;font-size:12px;line-height:1.6'>\""+data.quote+"\"</div></div>"
    +"<div style='display:flex;justify-content:space-between;padding-top:14px;border-top:1px solid rgba(255,255,255,0.05)'>"
    +"<div style='font-size:9px;color:rgba(255,255,255,0.18)'>FOR RESEARCH USE ONLY &middot; NOT INTENDED FOR HUMAN USE</div>"
    +"<div style='font-size:9px;color:rgba(255,255,255,0.2)'>alphaomegatides.com</div></div>"
    +"</div><script>window.onload=function(){window.print()}<\/script></body></html>";
  const blob=new Blob([html],{type:"text/html"});
  const url=URL.createObjectURL(blob);
  const a=document.createElement("a");
  a.href=url;a.download="alphaomegatides-"+key+"-research-guide.html";a.click();
  URL.revokeObjectURL(url);
}

function ComingSoonPage({go}:{go:(p:string)=>void}){
  ...
  const submit=async()=>{
    if(!form.firstName||!form.email){setErr("Please enter your name and email.");return;}
    if(!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(form.email)){setErr("Please enter a valid email.");return;}
    setLoading(true);setErr("");
    const s:NxgSignup={id:Date.now().toString(),firstName:form.firstName,lastName:form.lastName,email:form.email,phone:form.phone,interest:form.interest,productInterests:[],signedUpAt:new Date().toISOString(),source:"coming-soon"};
    saveNxgSignup(s);
    await notifyAdmin(s);
    setTimeout(()=>sendSurveyEmail(s),2000);
    setSent(true);setLoading(false);
  };
  if(sent)return(
    <div style={{minHeight:"100vh",background:"#0e0e0e",display:"flex",alignItems:"center",justifyContent:"center",padding:24,textAlign:"center"}}>
      <div><div style={{fontSize:"3rem",marginBottom:16}}>🧬</div>
        <h1 style={{fontFamily:"'Syne',sans-serif",fontWeight:900,fontSize:"1.8rem",color:"#3be8b0",marginBottom:12}}>You're on the list.</h1>
        <p style={{color:"rgba(255,255,255,0.5)",fontSize:"0.95rem",lineHeight:1.7,marginBottom:24,maxWidth:400}}>We'll reach out personally when we launch. Check your inbox — a research interest survey is on its way.</p>
        <p style={{color:"rgba(255,255,255,0.25)",fontFamily:"'Syne',sans-serif",fontStyle:"italic",fontSize:"0.85rem",marginBottom:28}}>"Where the tide turns for all."</p>
        <button onClick={()=>go("home")} style={{background:"transparent",border:"1px solid rgba(255,255,255,0.12)",borderRadius:100,padding:"11px 28px",color:"rgba(255,255,255,0.5)",fontFamily:"'DM Sans',sans-serif",fontWeight:600,cursor:"pointer",fontSize:"0.88rem"}}>← Continue Browsing</button>
      </div>
    </div>
  );
  return(
    <div style={{minHeight:"100vh",background:"#0e0e0e",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:"80px 20px 40px"}}>
      <div style={{maxWidth:480,width:"100%"}}>
        <div style={{textAlign:"center",marginBottom:32}}>
          <div style={{display:"inline-flex",alignItems:"center",gap:6,background:"rgba(59,232,176,0.08)",border:"1px solid rgba(59,232,176,0.2)",borderRadius:20,padding:"5px 14px",marginBottom:20}}>
            <div style={{width:6,height:6,borderRadius:"50%",background:"#3be8b0",animation:"pulse 2s infinite"}}/>
            <span style={{color:"#3be8b0",fontFamily:"'Syne',sans-serif",fontWeight:700,fontSize:"0.72rem",letterSpacing:"0.1em"}}>LAUNCHING SOON</span>
          </div>
          <h1 style={{fontFamily:"'Syne',sans-serif",fontWeight:900,fontSize:"clamp(1.8rem,5vw,2.4rem)",color:"#fff",lineHeight:1.1,marginBottom:14}}>Something extraordinary<br/><span style={{color:"#3be8b0"}}>is almost ready.</span></h1>
          <p style={{color:"rgba(255,255,255,0.45)",fontSize:"0.92rem",lineHeight:1.7}}>The most transparent, verified research peptide platform ever built. You found us early.</p>
        </div>
        <div style={{background:"rgba(255,255,255,0.03)",border:"1px solid rgba(255,255,255,0.08)",borderRadius:20,padding:"28px 24px"}}>
          <div style={{fontFamily:"'Syne',sans-serif",fontWeight:800,fontSize:"0.95rem",color:"#fff",marginBottom:4}}>Get Early Access</div>
          <p style={{color:"rgba(255,255,255,0.4)",fontSize:"0.8rem",marginBottom:20,lineHeight:1.5}}>Be the first to know when we launch.</p>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:10}}>
            {([["firstName","First Name *","John"],["lastName","Last Name","Smith"]] as [string,string,string][]).map(([k,lb,ph])=>(
              <div key={k}><label style={{display:"block",fontFamily:"'Syne',sans-serif",fontWeight:700,fontSize:"0.68rem",letterSpacing:"0.1em",color:"rgba(255,255,255,0.4)",textTransform:"uppercase",marginBottom:5}}>{lb}</label>
              <input value={(form as any)[k]} onChange={e=>upd(k,e.target.value)} placeholder={ph} style={{width:"100%",background:"rgba(255,255,255,0.05)",border:"1px solid rgba(255,255,255,0.1)",borderRadius:10,padding:"11px 13px",color:"#fff",fontFamily:"'DM Sans',sans-serif",fontSize:"0.88rem",outline:"none",boxSizing:"border-box"}}/></div>
            ))}
          </div>
          {([["email","Email Address *","your@email.com","email"],["phone","Phone (Optional)","+1 (555) 000-0000","tel"]] as [string,string,string,string][]).map(([k,lb,ph,tp])=>(
            <div key={k} style={{marginBottom:10}}><label style={{display:"block",fontFamily:"'Syne',sans-serif",fontWeight:700,fontSize:"0.68rem",letterSpacing:"0.1em",color:"rgba(255,255,255,0.4)",textTransform:"uppercase",marginBottom:5}}>{lb}</label>
            <input value={(form as any)[k]} onChange={e=>upd(k,e.target.value)} placeholder={ph} type={tp} style={{width:"100%",background:"rgba(255,255,255,0.05)",border:"1px solid rgba(255,255,255,0.1)",borderRadius:10,padding:"11px 13px",color:"#fff",fontFamily:"'DM Sans',sans-serif",fontSize:"0.88rem",outline:"none",boxSizing:"border-box"}}/></div>
          ))}
          <div style={{marginBottom:18}}><label style={{display:"block",fontFamily:"'Syne',sans-serif",fontWeight:700,fontSize:"0.68rem",letterSpacing:"0.1em",color:"rgba(255,255,255,0.4)",textTransform:"uppercase",marginBottom:5}}>Research Interest</label>
          <select value={form.interest} onChange={e=>upd("interest",e.target.value)} style={{width:"100%",background:"rgba(255,255,255,0.05)",border:"1px solid rgba(255,255,255,0.1)",borderRadius:10,padding:"11px 13px",color:form.interest?"#fff":"rgba(255,255,255,0.3)",fontFamily:"'DM Sans',sans-serif",fontSize:"0.88rem",outline:"none",boxSizing:"border-box"}}>
            <option value="">Select your primary interest...</option>
            {["Metabolic Research","Recovery Research","GH Research","Longevity Research","Cognitive Research","Dermal Research","Multiple areas","General interest"].map(o=><option key={o} value={o}>{o}</option>)}
          </select></div>
          {err&&<div style={{background:"rgba(255,107,107,0.08)",border:"1px solid rgba(255,107,107,0.2)",borderRadius:10,padding:"10px 14px",color:"#ff8a80",fontFamily:"'DM Sans',sans-serif",fontSize:"0.82rem",marginBottom:14}}>{err}</div>}
          <button onClick={submit} disabled={loading} style={{width:"100%",padding:"14px",background:loading?"rgba(59,232,176,0.3)":"#3be8b0",color:"#0e0e0e",border:"none",borderRadius:100,fontFamily:"'DM Sans',sans-serif",fontWeight:800,fontSize:"0.95rem",cursor:loading?"not-allowed":"pointer",display:"flex",alignItems:"center",justifyContent:"center",gap:8,boxShadow:"0 0 24px rgba(59,232,176,0.2)"}}>
            {loading?<><span style={{width:14,height:14,border:"2px solid #0e0e0e",borderTopColor:"transparent",borderRadius:"50%",animation:"spin 0.7s linear infinite",display:"inline-block"}}/> Sending...</>:<><span>🧬</span> Secure My Early Access</>}
          </button>
          <p style={{color:"rgba(255,255,255,0.2)",fontFamily:"'DM Sans',sans-serif",fontSize:"0.7rem",textAlign:"center",marginTop:12}}>No spam. Just a launch announcement.</p>
        </div>
        <div style={{textAlign:"center",marginTop:16}}><button onClick={()=>go("home")} style={{background:"transparent",border:"none",color:"rgba(255,255,255,0.25)",fontFamily:"'DM Sans',sans-serif",fontSize:"0.8rem",cursor:"pointer"}}>← Go back to browsing</button></div>
      </div>
    </div>
  );
}

function ComplianceBanner({go}:{go:(p:string)=>void}){
  const [vis,setVis]=useState(()=>{try{return!localStorage.getItem("nxg_compliance_ack");}catch{return true;}});
  if(!vis)return null;
  return(
    <div style={{position:"fixed",bottom:0,left:0,right:0,zIndex:9998,background:"rgba(10,10,10,0.98)",backdropFilter:"blur(12px)",borderTop:"1px solid rgba(255,255,255,0.08)",padding:"10px 20px",display:"flex",alignItems:"center",justifyContent:"space-between",gap:12,flexWrap:"wrap"}}>
      <p style={{color:"rgba(255,255,255,0.45)",fontFamily:"'DM Sans',sans-serif",fontSize:"0.72rem",lineHeight:1.5,flex:1,margin:0,minWidth:200}}>
        <span style={{color:"rgba(255,255,255,0.7)",fontWeight:600}}>Research Use Only. </span>All products are for in-vitro laboratory research only. Not for human use. Not FDA approved. Not intended to diagnose, treat, cure, or prevent any disease.
        {" "}<button onClick={()=>go("compliance")} style={{background:"transparent",border:"none",color:"#3be8b0",cursor:"pointer",fontFamily:"inherit",fontSize:"inherit",padding:0,textDecoration:"underline"}}>Full Notice</button>
      </p>
      <button onClick={()=>{localStorage.setItem("nxg_compliance_ack","1");setVis(false);}} style={{background:"rgba(59,232,176,0.1)",border:"1px solid rgba(59,232,176,0.25)",borderRadius:20,padding:"6px 16px",color:"#3be8b0",fontFamily:"'DM Sans',sans-serif",fontWeight:600,fontSize:"0.75rem",cursor:"pointer",flexShrink:0,whiteSpace:"nowrap"}}>
        I Understand — I Am a Researcher
      </button>
    </div>
  );
}

function ProductComplianceBox({productName}:{productName:string}){
  return(
    <div style={{background:"rgba(255,255,255,0.02)",border:"1px solid rgba(255,255,255,0.07)",borderRadius:12,padding:"11px 14px",marginBottom:14}}>
      <p style={{color:"rgba(255,255,255,0.35)",fontFamily:"'DM Sans',sans-serif",fontSize:"0.72rem",lineHeight:1.6,margin:0}}>
        <span style={{color:"rgba(255,255,255,0.5)",fontWeight:600}}>⚗️ Research Use Only. </span>
        {productName} is sold for in-vitro laboratory research only. Not evaluated by the FDA. Not intended to diagnose, treat, cure, or prevent any disease. Not for human or veterinary use.
      </p>
    </div>
  );
}

function CompliancePage({go}:{go:(p:string)=>void}){
  const sections=[
    {t:"Research Use Only (RUO)",b:"All products are For Research Use Only — intended exclusively for in-vitro laboratory research by qualified researchers. Not FDA-approved, not dietary supplements, not intended to diagnose, treat, cure, or prevent any disease. Purchase constitutes acknowledgment that you are a qualified researcher."},
    {t:"FDA Regulatory Notice",b:"Alphaomegatides does not market products for therapeutic or human use. All scientific information reflects published preclinical and in-vitro research data for educational reference only. We comply with applicable FDA regulations governing research-use-only chemical compounds."},
    {t:"FTC Compliance",b:"All claims reflect documented scientific literature from preclinical and in-vitro research. We make no unsubstantiated health claims. Research information does not constitute endorsement of human use and does not imply safety or efficacy for human use."},
    {t:"Intended Purchaser",b:"By purchasing you represent: (1) you are 21+; (2) a qualified researcher with legitimate scientific purposes; (3) you will use compounds exclusively for in-vitro lab research; (4) you will NOT administer to humans or animals; (5) you comply with all applicable laws."},
    {t:"Limitation of Liability",b:"TO THE MAXIMUM EXTENT PERMITTED BY LAW, ALPHAOMEGATIDES SHALL NOT BE LIABLE FOR ANY DAMAGES ARISING FROM USE OR MISUSE OF OUR PRODUCTS. You assume all risk associated with purchase, handling, storage, and use."},
    {t:"Scientific Information",b:"Research timelines and mechanism descriptions reflect published peer-reviewed preclinical studies for educational reference only. This is not medical advice or clinical guidance. Preclinical findings do not necessarily translate to human outcomes."},
  ];
  return(
    <div style={{minHeight:"100vh",background:"#0e0e0e",paddingTop:70,paddingBottom:80}}>
      <div style={{maxWidth:720,margin:"0 auto",padding:"40px 20px"}}>
        <div style={{marginBottom:32}}>
          <div style={{fontFamily:"'Syne',sans-serif",fontWeight:700,fontSize:"0.72rem",letterSpacing:"0.2em",color:"rgba(255,255,255,0.35)",textTransform:"uppercase",marginBottom:8}}>Legal & Regulatory</div>
          <h1 style={{fontFamily:"'Syne',sans-serif",fontWeight:900,fontSize:"clamp(1.6rem,4vw,2.2rem)",color:"#fff",marginBottom:12}}>Compliance & Regulatory Notice</h1>
        </div>
        {sections.map(({t,b})=>(
          <div key={t} style={{marginBottom:28,paddingBottom:28,borderBottom:"1px solid rgba(255,255,255,0.06)"}}>
            <h2 style={{fontFamily:"'Syne',sans-serif",fontWeight:800,fontSize:"1.05rem",color:"#fff",marginBottom:10}}>{t}</h2>
            <p style={{color:"rgba(255,255,255,0.5)",fontFamily:"'DM Sans',sans-serif",fontSize:"0.85rem",lineHeight:1.7}}>{b}</p>
          </div>
        ))}
        <div style={{background:"rgba(59,232,176,0.05)",border:"1px solid rgba(59,232,176,0.15)",borderRadius:12,padding:"16px 20px",marginBottom:24}}>
          <p style={{color:"rgba(255,255,255,0.4)",fontFamily:"'DM Sans',sans-serif",fontSize:"0.8rem",lineHeight:1.6,margin:0}}><span style={{color:"#3be8b0",fontWeight:600}}>Legal Disclaimer: </span>This notice does not constitute legal advice. Consult qualified legal counsel regarding regulatory status of specific compounds in your jurisdiction. Last updated: May 2026</p>
        </div>
        <button onClick={()=>go("home")} style={{background:"rgba(255,255,255,0.04)",border:"1px solid rgba(255,255,255,0.1)",borderRadius:100,padding:"12px 28px",color:"rgba(255,255,255,0.5)",fontFamily:"'DM Sans',sans-serif",fontWeight:600,cursor:"pointer",fontSize:"0.88rem"}}>← Back to Research</button>
      </div>
    </div>
  );
}

function AdminDashboard({go}:{go:(p:string)=>void}){
  const [authed,setAuthed]=useState(()=>sessionStorage.getItem("nxg_admin")==="1");
  const [pw,setPw]=useState("");
  const [pwErr,setPwErr]=useState(false);
  const [signups,setSignups]=useState<NxgSignup[]>([]);
  const [blasting,setBlasting]=useState(false);
  const [blastResult,setBlastResult]=useState<{sent:number;failed:number}|null>(null);
  const [filter,setFilter]=useState("all");
  const [search,setSearch]=useState("");
  const [tab,setTab]=useState<"customers"|"blast"|"survey">("customers");
  useEffect(()=>{if(authed)setSignups(getNxgSignups());},[authed]);
  const login=()=>{if(pw===ADMIN_PW){sessionStorage.setItem("nxg_admin","1");setAuthed(true);}else{setPwErr(true);setTimeout(()=>setPwErr(false),2000);}};
  const blast=async()=>{if(!window.confirm("Send launch email to ALL "+signups.length+" subscribers?"))return;setBlasting(true);const r=await sendLaunchBlast(signups);setBlastResult(r);setBlasting(false);};
  const resendSurveys=async()=>{const p=signups.filter(s=>!s.surveyCompletedAt);if(!window.confirm("Send survey to "+p.length+"?"))return;for(const s of p){await sendSurveyEmail(s);await new Promise(r=>setTimeout(r,300));}alert("Sent to "+p.length);};
  const exportCSV=()=>{const rows=[["First","Last","Email","Phone","Interest","Products","Signed Up","Survey"],...signups.map(s=>[s.firstName,s.lastName,s.email,s.phone||"",s.interest||"",(s.productInterests||[]).join("|"),s.signedUpAt,s.surveyCompletedAt||""])];const csv=rows.map(r=>r.map(c=>'"'+c+'"').join(",")).join("\n");const blob=new Blob([csv],{type:"text/csv"});const url=URL.createObjectURL(blob);const a=document.createElement("a");a.href=url;a.download="alphaomegatides-subscribers.csv";a.click();};
  const filtered=signups.filter(s=>{const mf=filter==="all"||(filter==="surveyed"&&s.surveyCompletedAt)||(filter==="unsurveyed"&&!s.surveyCompletedAt);const ms=!search||s.email.includes(search)||s.firstName.toLowerCase().includes(search.toLowerCase())||s.lastName.toLowerCase().includes(search.toLowerCase());return mf&&ms;});
  const topP=signups.flatMap(s=>s.productInterests||[]).reduce((a,p)=>{a[p]=(a[p]||0)+1;return a;},{} as Record<string,number>);
  const sortedP=Object.entries(topP).sort((a,b)=>b[1]-a[1]);
  if(!authed)return(
    <div style={{minHeight:"100vh",background:"#0e0e0e",display:"flex",alignItems:"center",justifyContent:"center",padding:20}}>
      <div style={{background:"#161616",border:"1px solid rgba(255,255,255,0.1)",borderRadius:20,padding:"36px 32px",maxWidth:360,width:"100%",textAlign:"center"}}>
        <div style={{fontSize:"2rem",marginBottom:12}}>🔐</div>
        <h2 style={{fontFamily:"'Syne',sans-serif",fontWeight:900,color:"#fff",fontSize:"1.3rem",marginBottom:6}}>Admin Access</h2>
        <p style={{color:"rgba(255,255,255,0.35)",fontSize:"0.82rem",marginBottom:24}}>Alphaomegatides Dashboard</p>
        <input value={pw} onChange={e=>setPw(e.target.value)} onKeyDown={e=>e.key==="Enter"&&login()} type="password" placeholder="Enter password" style={{width:"100%",background:pwErr?"rgba(255,107,107,0.08)":"rgba(255,255,255,0.05)",border:"1px solid "+(pwErr?"rgba(255,107,107,0.4)":"rgba(255,255,255,0.1)"),borderRadius:10,padding:"12px 16px",color:"#fff",fontFamily:"'DM Sans',sans-serif",fontSize:"0.9rem",outline:"none",boxSizing:"border-box",marginBottom:12}}/>
        {pwErr&&<div style={{color:"#ff8a80",fontSize:"0.78rem",marginBottom:12}}>Incorrect password</div>}
        <button onClick={login} style={{width:"100%",background:"#3be8b0",color:"#0e0e0e",border:"none",borderRadius:100,padding:"12px",fontFamily:"'DM Sans',sans-serif",fontWeight:800,fontSize:"0.95rem",cursor:"pointer",marginBottom:10}}>Enter Dashboard</button>
        <button onClick={()=>go("home")} style={{background:"transparent",border:"none",color:"rgba(255,255,255,0.25)",fontFamily:"'DM Sans',sans-serif",fontSize:"0.78rem",cursor:"pointer"}}>← Back to site</button>
      </div>
    </div>
  );
  return(
    <div style={{minHeight:"100vh",background:"#0a0a0a"}}>
      <div style={{background:"#161616",borderBottom:"1px solid rgba(255,255,255,0.08)",padding:"14px 24px",display:"flex",alignItems:"center",justifyContent:"space-between",position:"sticky",top:0,zIndex:100,flexWrap:"wrap",gap:8}}>
        <div style={{display:"flex",alignItems:"center",gap:10}}><span style={{fontSize:"1.2rem"}}>🧬</span><span style={{fontFamily:"'Syne',sans-serif",fontWeight:900,color:"#fff",fontSize:"1rem"}}>Alphaomegatides Admin</span></div>
        <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
          <button onClick={exportCSV} style={{background:"rgba(59,232,176,0.1)",border:"1px solid rgba(59,232,176,0.2)",borderRadius:20,padding:"6px 14px",color:"#3be8b0",fontFamily:"'DM Sans',sans-serif",fontWeight:600,fontSize:"0.78rem",cursor:"pointer"}}>⬇ Export CSV</button>
          <button onClick={()=>{sessionStorage.removeItem("nxg_admin");setAuthed(false);}} style={{background:"rgba(255,255,255,0.05)",border:"1px solid rgba(255,255,255,0.1)",borderRadius:20,padding:"6px 14px",color:"rgba(255,255,255,0.4)",fontFamily:"'DM Sans',sans-serif",fontWeight:600,fontSize:"0.78rem",cursor:"pointer"}}>Logout</button>
          <button onClick={()=>go("home")} style={{background:"rgba(255,255,255,0.05)",border:"1px solid rgba(255,255,255,0.1)",borderRadius:20,padding:"6px 14px",color:"rgba(255,255,255,0.4)",fontFamily:"'DM Sans',sans-serif",fontWeight:600,fontSize:"0.78rem",cursor:"pointer"}}>← Site</button>
        </div>
      </div>
      <div style={{maxWidth:1100,margin:"0 auto",padding:"28px 20px"}}>
        <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(170px,1fr))",gap:12,marginBottom:28}}>
          {[{lb:"Total Subscribers",v:signups.length,c:"#3be8b0",i:"👥"},{lb:"Survey Completed",v:signups.filter(s=>s.surveyCompletedAt).length,c:"#a855f7",i:"✅"},{lb:"Pending Survey",v:signups.filter(s=>!s.surveyCompletedAt).length,c:"#fbbf24",i:"⏳"},{lb:"Top Product",v:sortedP[0]?sortedP[0][0].toUpperCase():"—",c:"#60a5fa",i:"🔥"}].map(({lb,v,c,i})=>(
            <div key={lb} style={{background:"#161616",border:"1px solid "+c+"22",borderRadius:14,padding:"14px 16px"}}>
              <div style={{fontSize:"1.1rem",marginBottom:5}}>{i}</div>
              <div style={{fontFamily:"'Syne',sans-serif",fontWeight:900,fontSize:"1.4rem",color:c,marginBottom:2}}>{v}</div>
              <div style={{color:"rgba(255,255,255,0.35)",fontSize:"0.7rem"}}>{lb}</div>
            </div>
          ))}
        </div>
        <div style={{display:"flex",gap:8,marginBottom:20,flexWrap:"wrap"}}>
          {(["customers","blast","survey"] as const).map(t=>(
            <button key={t} onClick={()=>setTab(t)} style={{background:tab===t?"rgba(59,232,176,0.1)":"transparent",border:"1px solid "+(tab===t?"rgba(59,232,176,0.3)":"rgba(255,255,255,0.08)"),borderRadius:20,padding:"8px 18px",color:tab===t?"#3be8b0":"rgba(255,255,255,0.4)",fontFamily:"'DM Sans',sans-serif",fontWeight:700,fontSize:"0.82rem",cursor:"pointer"}}>
              {t==="customers"?"👥 Customers":t==="blast"?"🚀 Launch Blast":"📊 Survey Results"}
            </button>
          ))}
        </div>
        {tab==="customers"&&(
          <div>
            <div style={{display:"flex",gap:10,marginBottom:16,flexWrap:"wrap"}}>
              <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search name or email..." style={{flex:1,minWidth:200,background:"rgba(255,255,255,0.04)",border:"1px solid rgba(255,255,255,0.1)",borderRadius:10,padding:"10px 14px",color:"#fff",fontFamily:"'DM Sans',sans-serif",fontSize:"0.85rem",outline:"none"}}/>
              {["all","surveyed","unsurveyed"].map(f=><button key={f} onClick={()=>setFilter(f)} style={{background:filter===f?"rgba(59,232,176,0.08)":"transparent",border:"1px solid "+(filter===f?"rgba(59,232,176,0.2)":"rgba(255,255,255,0.08)"),borderRadius:20,padding:"8px 14px",color:filter===f?"#3be8b0":"rgba(255,255,255,0.35)",fontFamily:"'DM Sans',sans-serif",fontWeight:600,fontSize:"0.78rem",cursor:"pointer",textTransform:"capitalize"}}>{f}</button>)}
            </div>
            {filtered.length===0?<div style={{textAlign:"center",padding:"60px 20px",color:"rgba(255,255,255,0.2)",fontFamily:"'DM Sans',sans-serif"}}><div style={{fontSize:"2rem",marginBottom:8}}>📭</div>No subscribers yet.</div>:(
              <div style={{display:"flex",flexDirection:"column",gap:8}}>
                {filtered.map(s=>(
                  <div key={s.id} style={{background:"#161616",border:"1px solid rgba(255,255,255,0.07)",borderRadius:14,padding:"13px 16px",display:"flex",alignItems:"center",gap:12,flexWrap:"wrap"}}>
                    <div style={{width:34,height:34,borderRadius:"50%",background:"rgba(59,232,176,0.12)",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0,fontFamily:"'Syne',sans-serif",fontWeight:800,color:"#3be8b0",fontSize:"0.85rem"}}>{s.firstName?.[0]?.toUpperCase()}{s.lastName?.[0]?.toUpperCase()}</div>
                    <div style={{flex:1,minWidth:150}}><div style={{fontFamily:"'Syne',sans-serif",fontWeight:700,color:"#fff",fontSize:"0.88rem"}}>{s.firstName} {s.lastName}</div><div style={{color:"#3be8b0",fontSize:"0.75rem"}}>{s.email}</div>{s.phone&&<div style={{color:"rgba(255,255,255,0.3)",fontSize:"0.7rem"}}>{s.phone}</div>}</div>
                    <div style={{flex:1,minWidth:130}}><div style={{color:"rgba(255,255,255,0.3)",fontSize:"0.68rem",marginBottom:3}}>INTEREST</div><div style={{color:"rgba(255,255,255,0.6)",fontSize:"0.78rem"}}>{s.interest||"—"}</div></div>
                    <div style={{flex:1,minWidth:150}}><div style={{color:"rgba(255,255,255,0.3)",fontSize:"0.68rem",marginBottom:4}}>PRODUCTS</div><div style={{display:"flex",flexWrap:"wrap",gap:4}}>{(s.productInterests||[]).slice(0,4).map(p=><span key={p} style={{background:"rgba(168,85,247,0.1)",color:"#a855f7",border:"1px solid rgba(168,85,247,0.2)",borderRadius:20,padding:"2px 8px",fontSize:"0.6rem",fontWeight:600}}>{p.toUpperCase()}</span>)}{!s.productInterests?.length&&<span style={{color:"rgba(255,255,255,0.2)",fontSize:"0.7rem"}}>Survey pending</span>}</div></div>
                    <div style={{flexShrink:0,textAlign:"right"}}><span style={{background:s.surveyCompletedAt?"rgba(59,232,176,0.1)":"rgba(255,193,7,0.1)",color:s.surveyCompletedAt?"#3be8b0":"#fbbf24",border:"1px solid "+(s.surveyCompletedAt?"rgba(59,232,176,0.2)":"rgba(255,193,7,0.2)"),borderRadius:20,padding:"2px 8px",fontSize:"0.6rem",fontWeight:700}}>{s.surveyCompletedAt?"✅ Surveyed":"⏳ Pending"}</span><div style={{color:"rgba(255,255,255,0.2)",fontSize:"0.62rem",marginTop:3}}>{new Date(s.signedUpAt).toLocaleDateString()}</div></div>
                  </div>
                ))}
              </div>
            )}
        ...
        {tab==="blast"&&(
          <div style={{maxWidth:600}}>
            <div style={{background:"#161616",border:"1px solid rgba(255,107,107,0.2)",borderRadius:16,padding:"24px",marginBottom:14}}>
              <div style={{fontFamily:"'Syne',sans-serif",fontWeight:900,fontSize:"1.1rem",color:"#fff",marginBottom:8}}>🚀 Launch Email Blast</div>
              <p style={{color:"rgba(255,255,255,0.45)",fontSize:"0.85rem",lineHeight:1.6,marginBottom:20}}>Send the "We're Live" email to all <strong style={{color:"#fff"}}>{signups.length} subscribers</strong>.</p>
              {blastResult?<div style={{background:"rgba(59,232,176,0.08)",border:"1px solid rgba(59,232,176,0.2)",borderRadius:12,padding:"16px",textAlign:"center"}}><div style={{fontSize:"2rem",marginBottom:8}}>✅</div><div style={{fontFamily:"'Syne',sans-serif",fontWeight:800,color:"#3be8b0",fontSize:"1.2rem"}}>{blastResult.sent} sent{blastResult.failed>0&&<span style={{color:"#ff8a80",fontSize:"0.82rem"}}>, {blastResult.failed} failed</span>}</div></div>:(
                <button onClick={blast} disabled={blasting||signups.length===0} style={{width:"100%",padding:"16px",background:blasting?"rgba(59,232,176,0.3)":"linear-gradient(135deg,#ff6b6b,#a855f7,#3be8b0)",color:"#fff",border:"none",borderRadius:100,fontFamily:"'DM Sans',sans-serif",fontWeight:900,fontSize:"1rem",cursor:blasting||signups.length===0?"not-allowed":"pointer",display:"flex",alignItems:"center",justifyContent:"center",gap:8}}>
                  {blasting?<><span style={{width:16,height:16,border:"2px solid #fff",borderTopColor:"transparent",borderRadius:"50%",animation:"spin 0.7s linear infinite",display:"inline-block"}}/> Sending...</>:<><span>🚀</span> Send Launch Email to All {signups.length}</>}
                </button>
              )}
            </div>
            <div style={{background:"#161616",border:"1px solid rgba(168,85,247,0.2)",borderRadius:16,padding:"24px"}}>
              <div style={{fontFamily:"'Syne',sans-serif",fontWeight:900,fontSize:"1.1rem",color:"#fff",marginBottom:8}}>📋 Re-Send Survey</div>
              <p style={{color:"rgba(255,255,255,0.45)",fontSize:"0.85rem",lineHeight:1.6,marginBottom:16}}>Send to <strong style={{color:"#fff"}}>{signups.filter(s=>!s.surveyCompletedAt).length}</strong> who haven't responded.</p>
              <button onClick={resendSurveys} style={{width:"100%",padding:"14px",background:"rgba(168,85,247,0.1)",border:"1px solid rgba(168,85,247,0.3)",color:"#a855f7",borderRadius:100,fontFamily:"'DM Sans',sans-serif",fontWeight:700,fontSize:"0.9rem",cursor:"pointer"}}>📋 Send Survey to Remaining {signups.filter(s=>!s.surveyCompletedAt).length}</button>
            </div>
          </div>
        )}
        {tab==="survey"&&(
          <div style={{background:"#161616",border:"1px solid rgba(255,255,255,0.07)",borderRadius:16,padding:"24px"}}>
            <div style={{fontFamily:"'Syne',sans-serif",fontWeight:800,fontSize:"1rem",color:"#fff",marginBottom:16}}>📊 Product Interest Rankings</div>
            {sortedP.length===0?<p style={{color:"rgba(255,255,255,0.3)",fontSize:"0.85rem"}}>No survey responses yet.</p>:(
              <div style={{display:"flex",flexDirection:"column",gap:10}}>
                {sortedP.map(([id,count],i)=>{const pct=Math.round((count/Math.max(signups.length,1))*100);return(
                  <div key={id}><div style={{display:"flex",justifyContent:"space-between",marginBottom:4}}><span style={{color:"rgba(255,255,255,0.7)",fontSize:"0.85rem"}}>{i+1}. {id.toUpperCase()}</span><span style={{color:"#3be8b0",fontFamily:"'Syne',sans-serif",fontWeight:700,fontSize:"0.85rem"}}>{count} ({pct}%)</span></div><div style={{height:6,background:"rgba(255,255,255,0.05)",borderRadius:100}}><div style={{height:"100%",width:pct+"%",background:"linear-gradient(90deg,#3be8b0,#a855f7)",borderRadius:100}}/></div></div>
                );})}
              </div>
            )}
          </div>
...
function SocialProofTicker(){
  const items=["Researcher in California requested early access — 2 min ago","Research lab in Texas verified BPC-157 COA — 8 min ago","Anonymous researcher joined the waitlist — 14 min ago","University lab in New York requested NAD+ info — 22 min ago","Research team in Florida joined early access — 31 min ago"];
  const [idx,setIdx]=useState(0);const [vis,setVis]=useState(true);const [gone,setGone]=useState(false);
  useEffect(()=>{if(gone)return;const t=setInterval(()=>{setVis(false);setTimeout(()=>{setIdx(i=>(i+1)%items.length);setVis(true);},400);},5000);return()=>clearInterval(t);},[gone]);
  if(gone)return null;
  return(
    <div style={{position:"fixed",bottom:76,left:16,zIndex:500,maxWidth:280,background:"rgba(20,20,20,0.95)",border:"1px solid rgba(255,255,255,0.08)",borderRadius:12,padding:"10px 14px",backdropFilter:"blur(8px)",opacity:vis?1:0,transition:"opacity 0.4s",display:"flex",alignItems:"center",gap:10}}>
      <div style={{width:8,height:8,borderRadius:"50%",background:"#3be8b0",flexShrink:0,animation:"pulse 2s infinite"}}/>
      <span style={{color:"rgba(255,255,255,0.7)",fontFamily:"'DM Sans',sans-serif",fontSize:"0.72rem",lineHeight:1.4,flex:1}}>{items[idx]}</span>
      <button onClick={()=>setGone(true)} style={{background:"transparent",border:"none",color:"rgba(255,255,255,0.25)",cursor:"pointer",fontSize:"0.9rem",padding:0,flexShrink:0}}>×</button>
    </div>
  );
}
...
function ExitIntentPopup({onClose}:{onClose:()=>void}){
  ...
  return(
    <div onClick={onClose} style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.75)",backdropFilter:"blur(4px)",zIndex:9999,display:"flex",alignItems:"center",justifyContent:"center",padding:16}}>
      <div onClick={e=>e.stopPropagation()} style={{background:"#161616",borderRadius:24,border:"1px solid rgba(59,232,176,0.3)",padding:"36px 32px",maxWidth:440,width:"100%",boxShadow:"0 32px 80px rgba(0,0,0,0.8)",textAlign:"center",position:"relative"}}>
        <button onClick={onClose} style={{position:"absolute",top:14,right:14,background:"rgba(255,255,255,0.06)",border:"1px solid rgba(255,255,255,0.1)",borderRadius:"50%",width:28,height:28,display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer",color:"rgba(255,255,255,0.5)",fontSize:16}}>×</button>
        {done?(<div><div style={{fontSize:"2.5rem",marginBottom:12}}>🧬</div><div style={{fontFamily:"'Syne',sans-serif",fontWeight:800,fontSize:"1.3rem",color:"#3be8b0",marginBottom:8}}>You're in.</div><p style={{color:"rgba(255,255,255,0.5)",fontSize:"0.9rem"}}>We'll notify you the moment we launch.</p></div>):(
          <>
            <div style={{fontSize:"2rem",marginBottom:12}}>⚗️</div>
            <h2 style={{fontFamily:"'Syne',sans-serif",fontWeight:900,fontSize:"1.4rem",color:"#fff",marginBottom:8}}>Before you go...</h2>
            <p style={{color:"rgba(255,255,255,0.5)",fontSize:"0.9rem",marginBottom:24,lineHeight:1.6}}>Get notified the moment we launch + receive our complete research protocols library.</p>
            <div style={{display:"flex",gap:8,marginBottom:12}}>
              <input value={email} onChange={e=>setEmail(e.target.value)} placeholder="your@email.com" type="email" style={{flex:1,background:"rgba(255,255,255,0.06)",border:"1px solid rgba(255,255,255,0.12)",borderRadius:100,padding:"11px 16px",color:"#fff",fontFamily:"'DM Sans',sans-serif",fontSize:"0.9rem",outline:"none"}}/>
              <button onClick={()=>{if(email.includes("@")){const s:NxgSignup={id:Date.now().toString(),firstName:"",lastName:"",email,signedUpAt:new Date().toISOString(),source:"exit-intent"};saveNxgSignup(s);notifyAdmin(s);setDone(true);}}} style={{background:"#3be8b0",color:"#0e0e0e",border:"none",borderRadius:100,padding:"11px 20px",fontFamily:"'DM Sans',sans-serif",fontWeight:700,fontSize:"0.9rem",cursor:"pointer",whiteSpace:"nowrap"}}>Notify Me</button>
            </div>
            <button onClick={onClose} style={{background:"transparent",border:"none",color:"rgba(255,255,255,0.25)",fontSize:"0.75rem",cursor:"pointer",fontFamily:"'DM Sans',sans-serif"}}>No thanks</button>
          </>
        )}
      </div>
    </div>
...
function PageProgressBar(){
  const [pct,setPct]=useState(0);const [vis,setVis]=useState(false);
  useEffect(()=>{const h=()=>{const d=document.documentElement.scrollHeight-window.innerHeight;setVis(window.scrollY>100);if(d>0)setPct((window.scrollY/d)*100);};window.addEventListener("scroll",h,{passive:true});return()=>window.removeEventListener("scroll",h);},[]);
  if(!vis)return null;
  return <div style={{position:"fixed",top:60,left:0,right:0,height:2,zIndex:99,background:"rgba(255,255,255,0.04)"}}><div style={{height:"100%",width:pct+"%",background:"linear-gradient(90deg,#ff6b6b,#a855f7,#3be8b0)",transition:"width 0.1s linear"}}/></div>;
}

function ViewingNow({productId,color}:{productId:string;color:string}){
  const counts:Record<string,number>={glp3r:7,bpc157:12,tb500:5,glp1:9,nad:4,cjcipa:6,semax:3,selank:3,ghkcu:5,glow:4};
  const count=counts[productId]||Math.floor(Math.random()*4)+2;
  return(<div style={{display:"inline-flex",alignItems:"center",gap:6,background:color+"11",border:"1px solid "+color+"33",borderRadius:20,padding:"4px 12px",marginBottom:12}}><div style={{width:6,height:6,borderRadius:"50%",background:color,animation:"pulse 1.5s infinite"}}/><span style={{color,fontFamily:"'DM Sans',sans-serif",fontSize:"0.75rem",fontWeight:600}}>{count} researchers viewing</span></div>);
}
