    ...
    <div style={{position:"fixed",top:0,left:0,bottom:0,width:Math.min(320,window.innerWidth-40),background:"#111111",zIndex:997,transform:open?"translateX(0)":"translateX(-100%)",transition:"transform .28s cubic-bezier(.4,0,.2,1)",overflowY:"auto",boxShadow:open?"4px 0 32px rgba(0,0,0,0.18)":"none",display:"flex",flexDirection:"column"}}>
      ...
      <div style={{padding:"12px 0",flex:1}}>
        ...
        {[
          ...
          {icon:"ℹ️",label:"About Us",action:()=>{go("about");close();}},
          {icon:"📓",label:"Research Journal",action:()=>{go("journal");close();}},
          {icon:"🧬",label:"Stack Checker",action:()=>{go("stack");close();}},
          {icon:"⭐",label:"Loyalty Rewards",action:()=>{go("loyalty");close();}},
          {icon:"🎁",label:"Refer & Earn",action:()=>{go("referral");close();}},
...
function AnnouncementBar() {
  ...
  return (
    <div style={{background:"linear-gradient(90deg,#0d0d1a,#0a1a14,#0d0d1a)",borderBottom:"none",padding:"8px 20px",display:"flex",alignItems:"center",justifyContent:"center",gap:12,position:"relative",zIndex:996}}>
...
function DosingCalculatorPage({go}:{go:Function}){
  ...
  const [compound,setCompound]=React.useState("BPC-157");
  const [weight,setWeight]=React.useState("");
  const [weightUnit,setWeightUnit]=React.useState("kg");
  const [vialConc,setVialConc]=React.useState("");
  const [concUnit,setConcUnit]=React.useState("mcg/mL");
  const [customDose,setCustomDose]=React.useState("");

  const data=DOSING_DATA[compound];
  const weightKg=weight?(weightUnit==="lb"?parseFloat(weight)*0.453592:parseFloat(weight)):null;

  function calcVolume(doseStr:string):string|null{
    if(!weightKg||!vialConc||!doseStr) return null;
    ...
    const conc=parseFloat(vialConc);
    if(!conc) return null;
    const vol=(totalDose/conc)*1000; // convert to mL (conc in mcg/mL or mg/mL)
    return vol<0.01?"<0.01":vol.toFixed(3);
  }

  return <div style={{maxWidth:780,margin:"0 auto",padding:"24px 20px 80px"}}>
    <div style={{textAlign:"center",marginBottom:28}}>
      <h1 style={{fontFamily:"'Syne',sans-serif",fontWeight:800,fontSize:"clamp(1.6rem,4vw,2.1rem)",margin:"0 0 8px"}}>Research Dosing Calculator</h1>
      <p style={{color:muted,fontSize:"0.88rem",maxWidth:520,margin:"0 auto"}}>For research protocol planning only. All doses reference published animal study literature. Not for human use.</p>
    </div>

    <div style={{background:card,borderRadius:20,padding:"24px",marginBottom:20,boxShadow:"0 2px 12px rgba(0,0,0,0.08)"}}>
      <h3 style={{fontFamily:"'Syne',sans-serif",fontWeight:700,margin:"0 0 20px"}}>Protocol Parameters</h3>
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:14,marginBottom:14}}>
        <div style={{gridColumn:"1/-1"}}>
          <label style={{fontSize:"0.8rem",color:muted,display:"block",marginBottom:6}}>Compound</label>
          <select value={compound} onChange={e=>setCompound(e.target.value)} style={{...inputStyle,cursor:"pointer"}}>
            {Object.keys(DOSING_DATA).map(c=><option key={c} value={c}>{c}</option>)}
          </select>
        </div>
        <div>
          <label style={{fontSize:"0.8rem",color:muted,display:"block",marginBottom:6}}>Subject Weight</label>
          <input type="number" value={weight} onChange={e=>setWeight(e.target.value)} placeholder="e.g. 300" style={inputStyle}/>
        </div>
        <div>
          <label style={{fontSize:"0.8rem",color:muted,display:"block",marginBottom:6}}>Weight Unit</label>
          <select value={weightUnit} onChange={e=>setWeightUnit(e.target.value)} style={{...inputStyle,cursor:"pointer"}}>
        ...
        <div>
          <label style={{fontSize:"0.8rem",color:muted,display:"block",marginBottom:6}}>Vial Concentration</label>
          <input type="number" value={vialConc} onChange={e=>setVialConc(e.target.value)} placeholder="e.g. 500" style={inputStyle}/>
        </div>
        <div>
          <label style={{fontSize:"0.8rem",color:muted,display:"block",marginBottom:6}}>Concentration Unit</label>
          <select value={concUnit} onChange={e=>setConcUnit(e.target.value)} style={{...inputStyle,cursor:"pointer"}}>
    ...
    {/* Compound info */}
    {data&&<div style={{background:card,borderRadius:20,padding:"24px",marginBottom:20,boxShadow:"0 2px 12px rgba(0,0,0,0.08)"}}>
      <h3 style={{fontFamily:"'Syne',sans-serif",fontWeight:700,margin:"0 0 6px"}}>{compound} — Literature Reference</h3>
      <p style={{color:muted,fontSize:"0.83rem",lineHeight:1.65,margin:"0 0 16px"}}>{data.notes}</p>
      <div style={{background:dark?"rgba(59,232,176,0.06)":"rgba(59,232,176,0.08)",borderRadius:12,padding:"12px 16px",marginBottom:16,fontSize:"0.82rem",color:muted}}>
        <strong style={{color:"#3be8b0"}}>Reconstitution note:</strong> {data.reconVol}
      </div>
      <h4 style={{fontFamily:"'Syne',sans-serif",fontWeight:700,fontSize:"0.9rem",margin:"0 0 12px"}}>Published Research Dose Ranges</h4>
      <div style={{display:"flex",flexDirection:"column" as const,gap:10}}>
        {data.researchDoses.map(rd=>{
          const vol=weightKg&&vialConc?calcVolume(rd.dose):null;
          return <div key={rd.label} style={{background:dark?"rgba(255,255,255,0.04)":"rgba(0,0,0,0.04)",borderRadius:12,padding:"12px 16px",display:"flex",justifyContent:"space-between",alignItems:"center",flexWrap:"wrap" as const,gap:8}}>
            <div>
              <div style={{fontWeight:600,fontSize:"0.85rem"}}>{rd.label}</div>
              <div style={{fontSize:"0.82rem",color:"#3be8b0",fontWeight:700}}>{rd.dose}</div>
            </div>
            {vol&&<div style={{textAlign:"right"}}>
              <div style={{fontSize:"0.75rem",color:muted}}>Volume to draw</div>
              <div style={{fontWeight:800,fontSize:"1.1rem",color:"#a855f7"}}>{vol} mL</div>
            </div>}
          </div>;
        })}
      </div>
      ...
      <div style={{marginTop:20,paddingTop:16,borderTop:"1px solid rgba(128,128,128,0.15)"}}>
        <label style={{fontSize:"0.82rem",color:muted,display:"block",marginBottom:8}}>Custom Dose (per kg)</label>
        <div style={{display:"flex",gap:10,alignItems:"center"}}>
          <input type="text" value={customDose} onChange={e=>setCustomDose(e.target.value)} placeholder={`e.g. 50 (${data.unit}/kg)`}
            style={{...inputStyle,flex:1}}/>
          {weightKg&&vialConc&&customDose&&(()=>{
            const vol=calcVolume(customDose);
            return vol?<div style={{textAlign:"center",background:"rgba(168,85,247,0.1)",border:"1px solid rgba(168,85,247,0.3)",borderRadius:10,padding:"8px 16px",whiteSpace:"nowrap" as const}}>
              <div style={{fontSize:"0.72rem",color:muted}}>Volume</div>
              <div style={{fontWeight:800,color:"#a855f7"}}>{vol} mL</div>
            </div>:null;
          })()}
        </div>
      </div>
    ...
    <div style={{background:"rgba(255,107,107,0.06)",border:"1px solid rgba(255,107,107,0.2)",borderRadius:16,padding:"16px 20px"}}>
      <p style={{color:muted,fontSize:"0.78rem",margin:0,lineHeight:1.65,fontStyle:"italic"}}>
        <strong style={{color:"#ff6b6b"}}>Research Use Only.</strong> All dosing information is derived from published animal research literature and is provided for research protocol design and reference purposes only. This calculator does not constitute medical advice and is not intended for human dosing guidance. Always consult primary source literature before designing experiments. All compounds sold by Alphaomegatides are for in vitro and laboratory research use only.
      </p>
    </div>
  </div>;
}
